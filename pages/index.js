...
export default function WorkoutScannerApp() {
  ...
  const captureAndScan = async () => {
    ...
    try {
      ...
      canvas.toBlob(async (blob) => {
        try {
          ...
          if (text.trim()) {
            ...
            showNotification('Text scanned successfully! 🎉', 'success');
          } else {
            showNotification('No text found. Try again or use manual entry.', 'error');
          }
        } catch (ocrError) {
          console.error('OCR Error:', ocrError);
          showNotification('Scan failed. Using demo scan...', 'error');
          simulateOCR();
        } finally {
      ...
      showNotification('Capture failed. Using demo scan...', 'error');
      simulateOCR();
      setIsProcessing(false);
    }
  };
                ...
                <div className="flex gap-3">
                  {!isScanning ? (
                    ...
                    <>
                      ...
                      >
                        <Target size={20} />
                        {isProcessing ? 'Scanning...' : tesseractLoaded ? 'Scan Text' : 'Demo Scan'}
                      </button>
                      <button
                        onClick={stopCamera}
